...
namespace Nhom05DemoADB.Controllers
{
    public class SinhVienController : Controller
    {
        ...
        {
            List<SinhVienModel> sinhvien = sinhvienCollection.AsQueryable<SinhVienModel>().ToList();
            if (!String.IsNullOrEmpty(searchTerm))
            {
                sinhvien = sinhvienCollection.AsQueryable<SinhVienModel>().Where(x => x.HoTen.ToLower().Contains(searchTerm.ToLower())).ToList();
            }
